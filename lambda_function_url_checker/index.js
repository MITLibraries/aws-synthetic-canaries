/**
 * We conditionally require Synthetics and SyntheticsLogger in case we are
 * running locally and can't get them.
 */
let synthetics;
let log;
 ...
 */
if (process.env.NODE_ENV === "test" || process.env.WORKSPACE === "local") {
...
const main = async () => {
  ...
  if (!payloadEnv)
    throw new Error("Environment variable CANARY_PAYLOAD is not set.");
